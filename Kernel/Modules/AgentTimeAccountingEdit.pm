...
sub PreRun {
        ...
        )
    {
        ...
        return $Kernel::OM->Get('Kernel::Output::HTML::Layout')->Redirect( OP => 'Action=AgentTimeAccountingEdit' );
    }
...
sub Run {
    ...
    if ( $Self->{Subaction} eq 'Delete' ) {
        ...
        return $LayoutObject->NoPermission( WithHeader => 'yes' ) if !$Self->{AccessRo};
        ...
        for my $ID ( 1 .. $Param{RecordsNumber} ) {
            ...
            for my $Parameter (qw(ProjectID ActionID Remark StartTime EndTime Period)) {
                ...
                if ( $Param{$Parameter} ) {
                    ...
                    $ParamRef = $Kernel::OM->Get('Kernel::System::CheckItem')->StringClean(
                        StringRef => $ParamRef,
                        TrimLeft  => 1,
                        TrimRight => 1,
                    );
    ...
    for my $ID ( 1 .. $Param{RecordsNumber} ) {
        ...
        if ( $Errors{$ErrorIndex} && $Errors{$ErrorIndex}{StartTimeInvalid} ) {
            if ( scalar @{ $Errors{$ErrorIndex}{StartTimeServerErrorBlock} } > 0 ) {
                while ( @{ $Errors{$ErrorIndex}{StartTimeServerErrorBlock} } ) {
                    ...
                    $LayoutObject->Block( Name => $ServerErrorBlockName );
                }
            }
            else {
                $LayoutObject->Block( Name => 'StartTimeGenericServerError' );
            }
        }
        if ( $Errors{$ErrorIndex} && $Errors{$ErrorIndex}{EndTimeInvalid} ) {
            if ( scalar @{ $Errors{$ErrorIndex}{EndTimeServerErrorBlock} } > 0 ) {
                ...
                $LayoutObject->Block( Name => 'EndTimeGenericServerError' );
            }
        }
        ...
        if ( $Errors{$ErrorIndex} && $Errors{$ErrorIndex}{PeriodInvalid} ) {
            if ( scalar @{ $Errors{$ErrorIndex}{PeriodServerErrorBlock} } > 0 ) {
                ...
                $LayoutObject->Block( Name => 'PeriodGenericServerError' );
            }
        }
        else {
            $LayoutObject->Block( Name => 'PeriodGenericServerError' );
